...
func TestIsTorrentCompleteUsesCompletionOn(t *testing.T) {
	...
	torrent := &qbt.Torrent{
		Hash:         qbt.Ptr("abc"),
		Name:         qbt.Ptr("Example"),
		CompletionOn: qbt.Ptr(int64(123)),
		Progress:     qbt.Ptr(float64(0.12)),
		State:        qbt.Ptr(qbt.StateCheckingResumeData),
	}
...
func TestHandleCompletionUpdatesFiresOnceWhenCompletionOnAppears(t *testing.T) {
	...
	client.handleCompletionUpdates(&qbt.MainData{
		Torrents: map[string]qbt.Torrent{
			"def": {
				Hash:         qbt.Ptr("def"),
				Name:         qbt.Ptr("Still downloading"),
				CompletionOn: qbt.Ptr(int64(-1)),
				Progress:     qbt.Ptr(float64(0.50)),
	...
	client.handleCompletionUpdates(&qbt.MainData{
		Torrents: map[string]qbt.Torrent{
			"def": {
				Hash:         qbt.Ptr("def"),
				Name:         qbt.Ptr("Done now"),
				CompletionOn: qbt.Ptr(int64(999)),
				Progress:     qbt.Ptr(float64(1.0)),
				State:        qbt.Ptr(qbt.StateUploading),
			},
		},
	...
	select {
	case torrent := <-seen:
		if qbt.Deref(torrent.Hash) != "def" {
			t.Fatalf("unexpected hash: %q", qbt.Deref(torrent.Hash))
		}
...
func requireNoTorrentEvent(t *testing.T, ch <-chan qbt.Torrent, d time.Duration) {
	...
	case torrent := <-ch:
		t.Fatalf("unexpected completion event: hash=%q name=%q state=%q completionOn=%d",
			...
			qbt.Deref(torrent.Name),
			qbt.Deref(torrent.State),
			qbt.Deref(torrent.CompletionOn),
		)
	case <-time.After(d):
	}
}
